...
public class NewBeeMallGoodsController {
    ...
    public String editPhoto(HttpServletRequest request) {
        ...
        if (!CollectionUtils.isEmpty(firstLevelCategories)) {
            ...
            if (!CollectionUtils.isEmpty(secondLevelCategories)) {
                ...
                request.setAttribute("thirdLevelCategories", thirdLevelCategories);
                request.setAttribute("path", "goods-edit");
                return "admin/newbee_mall_goods_editPhoto";
            }
        }
        return "error/error_5xx";
    }
    ...
    @ResponseBody
    public Result insert(@RequestBody NewBeeMallGoods newBeeMallGoods) {
        boolean imgFlag;
        String result;
        if (Objects.isNull(newBeeMallGoods.getGoodsId())
                || StringUtils.isEmpty(newBeeMallGoods.getGoodsName())
        ...
        imgFlag = newBeeMallGoods.getImgFlag();

        if (imgFlag = true) {
            result = newBeeMallGoodsService.insertGoodsPhoto(newBeeMallGoods);

        } else {
            result = newBeeMallGoodsService.insertGoodsLargePhoto(newBeeMallGoods);
